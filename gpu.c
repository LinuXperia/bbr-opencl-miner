...
enum platform_type PrintPlatformInfo(unsigned int id, cl_platform_id platformId)
{
	...
	applog(LOG_INFO, "[Thread %u] Platform: %s", id, sInfo);
...
GPU* initGPU(uint32_t device_index, uint32_t id, uint32_t type) {
	...
	if (gpu->device == NULL) {
		applog(LOG_INFO, "[Thread %u] Device not found", id);
		return NULL;
	}
...
{
	if (opt_debug)
		applog(LOG_INFO, "[Thread %u] run work = %u, offset = %u", gpu->threadNumber, work_size, offset);
            ...
            *nonceptr = found_nonce;
            ...
            if (unlikely(hash[7] < ptarget[7])) {
                *hashes_done = n - first_nonce + opt_work_size;
            	...
            	applog(LOG_ERR, "[Thread %u] share doesn't validate on CPU, hash=%08x, target=%08x", gpu->threadNumber, hash[7], ptarget[7]);
    	}
    	n += opt_work_size;
